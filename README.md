## 前言
本项目旨在提供便捷的方式用于编译各种矿渣盒子、外贸机顶盒、路由器、X86的openwrt固件，支持使用github action流水线编译，以及在本地使用docker容器进行编译。
亮点在于本地编译时无需安装和解决各种复杂的环境依赖软件依赖，直接构建一个镜像和容器进行编译，编译完成后可以直接使用docker rm删除镜像和容器，即用即走，不对环境造成任何污染。

## 支持的能力
- 支持Arm平台、X86平台、MIPS平台的固件编译
- 支持使用Github Action编译，使用Docker进行编译，保证与本地编译环境产物一模一样
- 支持本地编译，本地编译无需准备复杂的环境，只需要一个Linux虚拟机+安装了Docker即可
- 支持版本机制，即可以指定用哪个Commit的代码来进行编译
- 支持将固件安装到云服务器
